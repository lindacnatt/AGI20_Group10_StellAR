...
public class CelestialBodyGenerator : MonoBehaviour
{
	...
	public MoonShading shading;
	public MoonShape shape;
	...
	void HandleGameModeGeneration()
	{
		if (CanGenerateMesh())
		{
			...
			{
				Vector2 lodTerrainHeightMinMax = GenerateTerrainMesh(ref lodMeshes[i], resolution);
			...
			terrainMatInstance = new Material(shading.terrainMaterial);
			shading.Initialize(shape);
			shading.SetTerrainProperties(terrainMatInstance, heightMinMax, BodyScale);
	...
	void HandleEditModeGeneration()
	{
		...
		if (CanGenerateMesh())
		{
			...
			if (shapeSettingsUpdated)
			{
				...
				heightMinMax = GenerateTerrainMesh(ref previewMesh, resolution);
			...
			else if (shadingNoiseSettingsUpdated)
			{
				...
				shading.Initialize(shape);
				Vector4[] shadingData = shading.GenerateShadingData(vertexBuffer);
		...
		if (shading)
		{
			...
			shading.Initialize(shape);
			shading.SetTerrainProperties(shading.terrainMaterial, heightMinMax, BodyScale);
		}
	...
	void OnValidate()
	{
		if (shading || shape)
		{
			...
			if (shading)
			{
				shading.OnSettingChanged -= OnShadingNoiseSettingChanged;
				shading.OnSettingChanged += OnShadingNoiseSettingChanged;
			}
		}
	...
	{
		GameObject terrainHolder = GetOrCreateMeshObject("Terrain Mesh", previewMesh, shading.terrainMaterial);
	}
	...
	{
		return ComputeHelper.CanRunEditModeCompute && shape && shape.heightMapCompute;
	}
